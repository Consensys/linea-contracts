...
import { TestZkEvmV1, TestZkEvmV1__factory } from "../typechain-types";

describe("ZK EVM V1 contract", () => {
  let zkEvm: TestZkEvmV1;
  let multiRollupZkEvm: TestZkEvmV1;
  ...
  describe("Initialisation", () => {
    const zkEvmInterface = TestZkEvmV1__factory.createInterface();
    ...
    it("Should assign the VERIFIER_SETTER_ROLE to operator addresses", async () => {
      const { zkEvm } = await loadFixture(deployZkEvmFixture);
      expect(await zkEvm.hasRole(VERIFIER_SETTER_ROLE, securityCouncil.address)).to.be.true;
    });
  ...
  describe("When not paused", () => {
    describe("Change verifier address", () => {
      ...
      it("Should revert if the address being set is the zero address", async () => {
        await zkEvm.connect(securityCouncil).grantRole(VERIFIER_SETTER_ROLE, securityCouncil.address);
        await expect(zkEvm.connect(securityCouncil).setVerifierAddress(ADDRESS_ZERO, 2)).to.be.revertedWithCustomError(
          zkEvm,
    ...
    describe("finalizeBlocks with light proof", () => {
      ...
      it("Should fail to process with empty block data", async () => {
        for (const tx of getTransactionsToBeDecoded(blocks)) {
          await zkEvm.addL1L2MessageHash(tx);
        }

        await expect(
          zkEvm.connect(operator).finalizeBlocks([], proof, 0, parentStateRootHash, { gasLimit: 15_000_000 }),
        ).to.be.revertedWithCustomError(zkEvm, "EmptyBlockDataArray");
      });
  ...
  describe("Block processing directly", () => {
    describe("process block transactions", () => {
      it("Should succeed and emit expected L1L2MessagesReceivedOnL2 events", async () => {
        ...
        for (const block of blocksRollup1) {
          const tx = await zkEvm
            .connect(operator)
        ...
        for (const blockTx of getTransactionsToBeDecoded(blocksRollup1)) {
  ...
  describe("Pausing and unpausing", () => {
    it("Should Pause and unpause", async () => {
      await zkEvm.connect(securityCouncil).pauseByType(GENERAL_PAUSE_TYPE);
      let isPaused = await zkEvm.isPaused(GENERAL_PAUSE_TYPE);
      expect(isPaused).to.be.true;

      await zkEvm.connect(securityCouncil).unPauseByType(GENERAL_PAUSE_TYPE);
      isPaused = await zkEvm.isPaused(GENERAL_PAUSE_TYPE);
