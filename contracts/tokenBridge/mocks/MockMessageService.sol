...
contract MockMessageService is IMessageService {
  uint256 public constant callGasLimit = 1000000;
  address messageSender = address(0);

  function sendMessage(address _to, uint256 _fee, bytes calldata _calldata) external payable {
    require(msg.value >= _fee, "MessageService: Value too low");
    messageSender = msg.sender;
    uint256 _value = msg.value - _fee;
    (bool success, bytes memory result) = _to.call{ value: _value, gas: callGasLimit }(_calldata);
